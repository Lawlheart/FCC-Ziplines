...
var GameMap = React.createClass({
  ...
  render: function render() {
    ...
    return React.createElement(
      'div',
      { className: 'map', style: this.props.mapStyle },
      grid
    );
  }
});
...
var Main = React.createClass({
  ...
  randomLocations: function randomLocations(num) {
    var locations = [];
    for (var i = 0; i < num; i++) {
      locations.push([Math.floor(Math.random() * 30), Math.floor(Math.random() * 20)]);
    }
    return locations;
  },
  componentDidMount: function componentDidMount() {
  ...
  resolveMove: function resolveMove() {
  ...
  render: function render() {
    ...
    return React.createElement(
      ...
      React.createElement('div', { className: heroclass, onKeyDown: this.moveSprite, style: herostyle })
    );
  }
});
